...
SELECT
	CONCAT('Converted from IIS.',	CHAR(13)+CHAR(10), 'IIS ID: ',	[IIS_ID], CHAR(13)+CHAR(10), 'Data Consolidation Matching ID: ', [REGULATED_ENTITY_ID_SOURCE]) [description], 
...
(
 ...
 (
   SELECT CAT.FILE_NUMBER_NUM, CAT.ACTIVITY_TYPE_CD, CAT.ACTIVITY_TYPE_ENM, ISNULL(SUBCAT.SUBACTIVITY_TYPE_CDS,'N/A') SUBACTIVITY_TYPE_CDS, ISNULL(SUBCAT.SUBACTIVITY_TYPE_ENMS,'N/A') SUBACTIVITY_TYPE_ENMS
   FROM (
    SELECT YD101.FILE_NUMBER_NUM, TD045.ACTIVITY_TYPE_CD, TD045.ACTIVITY_TYPE_ENM
    FROM [dbo].[YD101_FILE_ACTIVITY_TYPE] YD101
    JOIN TD045_ACTIVITY_TYPE TD045 ON YD101.ACTIVITY_TYPE_CD = TD045.ACTIVITY_TYPE_CD
    WHERE ACTIVITY_TYPE_PARENT_CD IS NULL AND YD101.DATE_DELETED_DTE IS NULL AND TD045.DATE_DELETED_DTE IS NULL
    GROUP BY YD101.FILE_NUMBER_NUM, TD045.ACTIVITY_TYPE_CD, TD045.ACTIVITY_TYPE_ENM
   ) CAT
   LEFT JOIN
   (
    SELECT YD101.FILE_NUMBER_NUM, STRING_AGG ( ISNULL(TD045.ACTIVITY_TYPE_CD,'N/A'), ',') AS SUBACTIVITY_TYPE_CDS , STRING_AGG ( ISNULL(TD045.ACTIVITY_TYPE_ENM,'N/A'), ',') AS SUBACTIVITY_TYPE_ENMS
    FROM [dbo].[YD101_FILE_ACTIVITY_TYPE] YD101
    JOIN TD045_ACTIVITY_TYPE TD045 ON YD101.ACTIVITY_TYPE_CD = TD045.ACTIVITY_TYPE_CD
    WHERE ACTIVITY_TYPE_PARENT_CD IS NOT NULL AND YD101.DATE_DELETED_DTE IS NULL AND TD045.DATE_DELETED_DTE IS NULL
    GROUP BY YD101.FILE_NUMBER_NUM
   ) SUBCAT

   ON CAT.FILE_NUMBER_NUM = SUBCAT.FILE_NUMBER_NUM
...
FROM 
(
	select 20					IIS_ACTIVITY_TYPE_ID, 
	...
	select    21  , 'General Compliance Inspection'        , 26,    'Compliance Estimation Program (CEP)'                 ,    @CONST_OVERSIGHTTYPE_GCTARGETED       ,    'GC Targeted',    @CONST_RATIONALE_PLANNED   ,    'Planned'                              UNION
	select    21  , 'General Compliance Inspection'        , 28,    'Follow-up Inspection (Discretionary)'                ,    @CONST_OVERSIGHTTYPE_GCTARGETED       ,    'GC Targeted',    @CONST_RATIONALE_PLANNED   ,    'Planned'                              UNION
	select    21  , 'General Compliance Inspection'        , 27,    'Follow-up Inspection (Indicated)'                    ,    @CONST_OVERSIGHTTYPE_GCTARGETED       ,    'GC Targeted',    @CONST_RATIONALE_PLANNED   ,    'Planned'                              UNION
	select    21  , 'General Compliance Inspection'        , 33,    'Opportunity'                                         ,    '07966E1C-5F9C-EB11-B1AC-000D3AE92708',    'GC Opportunity'                      ,    @CONST_RATIONALE_UNPLANNED,    'Unplanned'    UNION
	select    21  , 'General Compliance Inspection'        , 25,    'Scheduled (Workplan / Risk Based - excluding others)',    'ABF3C4D9-269E-EB11-B1AC-000D3AE924D1',    'GC IPT',    @CONST_RATIONALE_PLANNED        ,    'Planned'                              UNION
	select    21  , 'General Compliance Inspection'        , 32,    'Triggered'                                           ,    '9DEE7316-5F9C-EB11-B1AC-000D3AE92708',    'GC Triggered',    @CONST_RATIONALE_UNPLANNED,    'Unplanned'                            UNION
	select    22  , 'MOC Facility Inspection (Planned)'    , NULL,  'all subcategory types'                               ,    'E99AB21B-279E-EB11-B1AC-000D3AE924D1',    'MOC Facility IPT'                    ,    @CONST_RATIONALE_PLANNED,    'Planned'        UNION
	select    23  , 'Consignment'                          , NULL,  'all subcategory types'                               ,    @CONST_OVERSIGHTTYPE_GCCONSIGNMENT     ,    'GC Consignment'                      ,    @CONST_RATIONALE_UNPLANNED,    'Unplanned'    UNION
	select    24  , 'Document Review'                      , NULL,  'n/a'                                                 ,    @CONST_OVERSIGHTTYPE_CIVDOCREVIEW      ,    'Civil Aviation Document Review'      ,    @CONST_RATIONALE_UNPLANNED,    'Unplanned'    UNION
	select    59  , 'MOC Facility Inspection - (Unplanned)', NULL,  'all subcategory types'                               ,    @CONST_OVERSIGHTTYPE_MOCOPPORTUNITY    ,    'MOC Facility Opportunity'            ,    @CONST_RATIONALE_UNPLANNED,    'Unplanned'    UNION
	select    NULL, 'NULL'									, NULL   , 'NULL'                                              ,    '1CC06B3F-5F9C-EB11-B1AC-000D3AE92708',    'GC Undeclared / Misdeclared'         ,    @CONST_RATIONALE_UNPLANNED,    'Unplanned'    UNION
	select    NULL, 'NULL'									, NULL   , 'NULL'                                              ,    'C1E09A33-279E-EB11-B1AC-000D3AE924D1',    'MOC Facility Follow-up'              ,    @CONST_RATIONALE_UNPLANNED,    'Unplanned'    UNION
	select    NULL, 'NULL'									, NULL   , 'NULL'                                              ,    @CONST_OVERSIGHTTYPE_MOCTARGETED      ,    'MOC Facility Targeted'               ,    @CONST_RATIONALE_PLANNED,    'Planned'        UNION
	select    NULL, 'NULL'									, NULL   , 'NULL'                                              ,    'EA95BC68-5F9C-EB11-B1AC-000D3AE92708',    'MOC Facility Triggered'              ,    @CONST_RATIONALE_UNPLANNED,    'Unplanned'
) T;

--only convert activities that have a mapping defined
DELETE FROM #ACTIVITY_TYPE_MAPPINGS WHERE IIS_ACTIVITY_TYPE_ID IS NULL;
SELECT * FROM #ACTIVITY_TYPE_MAPPINGS;

--create values for fiscal data
DROP TABLE IF EXISTS #T2;
SELECT T.*, FY.tc_tcfiscalyearid, FQ.tc_tcfiscalquarterid
INTO #T2
FROM (
 SELECT
  newid() ID,                    --WORK ORDER ID
  CONVERT(uniqueidentifier, @CONST_WORKORDERTYPE_INSPECTION) WORK_ORDER_TYPE,         --work order type "Inspection"
  ...
  ,PRIMARY_INSPECTOR_USER_ID owner              --@CONST_TDG_TEAMID [OWNER] --OWNER = primary inspector
  ...
  --,@CONST_TDG_TEAMID [OWNING_TEAM]         --OWNING TEAM = TDG Team
 ...
 FROM [19_IIS_INSPECTIONS] T1, #ACTIVITY_TYPE_MAPPINGS T2         
 ...
 WHERE 
 ROM_OVERSIGHT_TYPE_ID <> 'SITE_VISIT' AND
 ((T1.ACTIVITY_TYPE_CD = t2.IIS_ACTIVITY_TYPE_ID) AND IIS_ACTIVITY_SUB_TYPE_ID IS NULL) OR
 (T1.ACTIVITY_TYPE_CD = t2.IIS_ACTIVITY_TYPE_ID AND T1.SUBACTIVITY_TYPE_CDS = CAST(T2.IIS_ACTIVITY_SUB_TYPE_ID as varchar(10))) OR 
 (T1.ACTIVITY_TYPE_CD = t2.IIS_ACTIVITY_TYPE_ID AND CAST(T2.IIS_ACTIVITY_SUB_TYPE_ID as varchar(10)) IS NULL) OR 
 (T1.ACTIVITY_TYPE_CD = t2.IIS_ACTIVITY_TYPE_ID AND CHARINDEX(CAST(T2.IIS_ACTIVITY_SUB_TYPE_ID as varchar(10)),T1.SUBACTIVITY_TYPE_CDS) > 0)
 AND id IS NOT NULL
) T
JOIN [dbo].[13_FISCAL_YEAR] FY ON FY.TC_NAME = T.FISCAL_YEAR
JOIN [dbo].[14_FISCAL_QUARTER] FQ ON FQ.TC_NAME = T.FISCAL_QUARTER AND FY.tc_tcfiscalyearid = FQ.tc_tcfiscalyearid
...
SELECT
 ...
 ,OWNING_TEAM                                                                        [OWNING_TEAM]          --OWNING TEAM = TDG Team
...
(
 SELECT 
  CASE WHEN BR.userid IS NULL    THEN @CONST_TDG_TEAMID ELSE BR.userid    END OWNER_ID,
  CASE WHEN BR.bookableresourceid IS NULL THEN @CONST_TDGCORE_BOOKABLE_RESOURCE_ID ELSE BR.bookableresourceid END BOOKABLE_RESOURCE_ID,
  CASE WHEN BR.userid IS NULL    THEN 'team'         ELSE 'systemuser'   END OWNER_TYPE,
  CASE WHEN BR.userid IS NULL    THEN @CONST_TDG_TEAMID ELSE NULL     END OWNING_TEAM,
  CASE WHEN BR.userid IS NULL    THEN NULL         ELSE BR.userid    END OWNING_USER,
 ...
 LEFT JOIN [dbo].[bookableresource] BR ON UPPER(TRIM(CONCAT(CONCAT(WP.InspectorFirstName, ' '), WP.InspectorLastName))) = UPPER(BR.name)
...
SELECT 
 ...
 @CONST_TDG_BUSINESSUNITID            [OWNING_BUSINESS_UNIT] --TDG Business Unit
 ,@CONST_TDG_TEAMID            [OWNING_TEAM]   --OWNING TEAM = TDG Team
 ,@CONST_TDG_TEAMID            [OWNER]     --OWNER = primary inspector
...
--set the parent company of the contact records;
update [11_CONTACT]
SET
accountid = T1.Id,
company = t1.Id
